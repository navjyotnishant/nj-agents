...
CONVENTIONS-testing.md      # testing-class shared rules (§T1–T12); skills in NAV-161
...
- **Testing class** — *writes test source into the repo **and executes it** against a
  ...
  nothing, `/test-gap-finder` explicitly never writes tests. Shared rules: `CONVENTIONS-testing.md` (§T1–T12). Three of those are enforced by `check.sh`:
