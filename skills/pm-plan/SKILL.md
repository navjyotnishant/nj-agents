...
**destination team/project/board** — never assume it (§P7).

Resolve the **project** once, here, and set it on **every item in the tree** (§P2b):
an epic in the project whose stories sit outside it looks complete in the tracker and
is broken in every project-scoped report. If the tree is also reported on through a
warehouse, check the grouping survives the sync — some connectors drop projects.

Note the tracker's shape so the plan fits it. On **GitHub** the tree is native: Epic = Issue `[Epic] …`, Stories and
