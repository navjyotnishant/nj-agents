...
- **`bin/nj-agents-e2e` fix: `--max-budget-usd` no longer passed under
  subscription auth.** It always forwarded that flag to the underlying `claude -p`
  call, but the flag's own `--help` text says it tracks "dollar amount to spend
  on **API** calls" — meaningless under a Claude subscription token (`claude auth
  status`: `authMethod: claude.ai`, no API key, no per-call dollar cost). Now only
  added when `ANTHROPIC_API_KEY` is actually set. Verified both branches via
  `--dry-run`. (Wiring this into a real CI workflow was tried and reverted: GitHub
  Actions runs on a hosted runner with no access to this machine's local
  subscription session, so it would need its own `CLAUDE_CODE_OAUTH_TOKEN` secret
  — deliberately not added; `bin/nj-agents-e2e` stays a local-only tool for now.)
