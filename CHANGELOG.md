...
- **`bin/nj-agents-e2e` wired into a real CI workflow** (`.github/workflows/e2e-smoke.yml`).
  ...
  and on changes to the e2e-suite/e2e-run/test-triage skills or the
  `failure-triager` agent. **Authenticates via `CLAUDE_CODE_OAUTH_TOKEN`** (a
  Claude subscription token from `claude setup-token`, stored as a repo secret),
  not a metered `ANTHROPIC_API_KEY` — matches this session's own auth (`claude
  auth status`: `authMethod: claude.ai`, no API key). Also fixed a related bug
  in `bin/nj-agents-e2e` itself: it always passed `--max-budget-usd` to the
  underlying `claude -p` call, a flag whose own `--help` text says it tracks
  "dollar amount to spend on **API** calls" — meaningless under a subscription
  token with no per-call dollar cost. Now only added when `ANTHROPIC_API_KEY` is
  actually set. Verified both branches via `--dry-run` (flag correctly included
  when the key is set, omitted when absent).
